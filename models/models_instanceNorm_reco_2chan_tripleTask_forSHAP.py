...
class ResNet34(nn.Module):
    def __init__(self, in_channels, resblock, outputs=6, returnTask=0):
        super().__init__()
    ...
    def forward(self, X):
        ...
        if self.returnTask == 2:
          return nn.Sigmoid()(XregP).squeeze()
        elif self.returnTask == 1:
          return nn.Sigmoid()(XregC).squeeze()
        else:
          return self.logSoftmax(Xclass)

        #outputs = [self.logSoftmax(Xclass), nn.Sigmoid()(XregC).squeeze(), nn.Sigmoid()(XregP).squeeze()]
        #return outputs
