...
def getTarget(pdgs, purities):
  target = [0.,0.,0.,0.,0.]
  for i, pdg in enumerate(pdgs):
    pClass = getClass(pdg)
    if pClass < 5:
      target[pClass] += purities[i]
  targetSum = 0.
  for p in target:
    targetSum += p
  #if abs(targetSum - 1.0) > 1e-6:
  if targetSum != 1.:
...
def getDominantClass(pdgs, purities):
  target = getTarget(pdgs, purities)
  iMax = 0
  pMax = -1.
  for i in range(len(target)):
    if target[i] > pMax:
...
class ProngDataset(Dataset):
    ...
    def __init__(self, rootfile, transformations=None, clip=1000.0):
        ...
        pdgs = self.tree["pdgs"].array(library="np")
        purities = self.tree["purities"].array(library="np")
        self.classes = np.array([getDominantClass(pdgs[i], purities[i]) for i in range(len(pdgs))])
    ...
    def __getitem__(self, item):
        ...
        arrays = self.tree.arrays(["pdgs", "purities", "plane0pix_row", "plane0pix_col", "plane0pix_val",
        ...
        target = getTarget(arrays["pdgs"][0], arrays["purities"][0])
        
        if self.transforms is not None:
            image = self.transforms(image)
        
        return torch.clamp(image, max=self.clipVal), torch.tensor(target)
...
class ProngDatasetPl2(Dataset):
    ...
    def __getitem__(self, item):
        ...
        arrays = self.tree.arrays(["pdgs", "purities", "plane2pix_row", "plane2pix_col", "plane2pix_val",
                                   "raw_plane2pix_row", "raw_plane2pix_col", "raw_plane2pix_val"],
                                  library="np", entry_start=item, entry_stop=item+1)
