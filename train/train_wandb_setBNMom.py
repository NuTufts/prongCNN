...
parser.add_argument("-bnm", "--batchNormMom", type=float, default=0.1, help="momentum parameter for batch norm layers")
...
parser.add_argument("--batchNormCMA", action="store_true", help="use cumulative moving average for batch norm layers")
...
if args.plane2only:
    ...
    test_dataloader = DataLoader(test_dataset, batch_size=args.batch_size_val, drop_last=True, shuffle=True, num_workers=args.num_workers)

    if args.resnet18:
        if args.batchNormCMA:
            model = ResNet18Pl2(1, ResBlock, outputs=5, batchMom=None)
        else:
            model = ResNet18Pl2(1, ResBlock, outputs=5, batchMom=args.batchNormMom)
    else:
        if args.batchNormCMA:
            model = ResNet34Pl2(1, ResBlock, outputs=5, batchMom=None)
        else:
            model = ResNet34Pl2(1, ResBlock, outputs=5, batchMom=args.batchNormMom)
    ...
    if args.resnet18:
        if args.batchNormCMA:
            model = ResNet18(1, ResBlock, outputs=5, batchMom=None)
        else:
            model = ResNet18(1, ResBlock, outputs=5, batchMom=args.batchNormMom)
    else:
        if args.batchNormCMA:
            model = ResNet34(1, ResBlock, outputs=5, batchMom=None)
        else:
            model = ResNet34(1, ResBlock, outputs=5, batchMom=args.batchNormMom)
    if not args.singleGPU:
        model = nn.DataParallel(model)
    model.to(args.device)
