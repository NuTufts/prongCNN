...
parser.add_argument("-if", "--recofile", required=True, type=str, help="input kpsreco file")
parser.add_argument("-it", "--truthfile", required=True, type=str, help="input merged_dlreco file")
parser.add_argument("-ig", "--genfiles", required=True, type=str, nargs='+', help="input single particle larcv files")
parser.add_argument("-r", "--run", required=True, type=int, help="run for input event")
parser.add_argument("-sr", "--subrun", required=True, type=int, help="subrun for input event")
parser.add_argument("-e", "--event", required=True, type=int, help="event for input event")
parser.add_argument("-trm", "--trackid_rm", required=True, type=int, help="track id to remove from context image")
parser.add_argument("-trm2", "--trackid_rm2", type=int, default=-1, help="2nd track id to remove from context image")
parser.add_argument("-ti", "--track_index", type=int, default=0, help="index for track corresponding to input prong")
parser.add_argument("-si", "--shower_index", type=int, default=-1, help="index for shower corresponding to input prong")
parser.add_argument("-o", "--outfile", type=str, default="prongCNN_partGenSub_images_file.root", help="out file name")
...
parser.add_argument("--contextSub", action="store_true", help="replace context particle instead of input prong")
...
outFile = rt.TFile(args.outfile.replace(".root",f"_r{args.run}_sr{args.subrun}_e{args.event}.root"),"RECREATE")
...
try:
  kpsfile = rt.TFile(args.recofile)
  kpst = kpsfile.Get("KPSRecoManagerTree")
  foo = kpst.GetEntries()
except:
  sys.exit("file "+args.recofile+" is bad")

try:
  ioll = larlite.storage_manager(larlite.storage_manager.kREAD)
  ioll.add_in_filename(args.truthfile)
  ioll.open()
  foo = ioll.get_entries()
except:
  sys.exit("file "+args.truthfile+" is bad")

iolcv = larcv.IOManager(larcv.IOManager.kREAD, "larcv", larcv.IOManager.kTickBackward)
iolcv.add_in_file(args.truthfile)
...
for iE in range(ioll.get_entries()):
  ...
  #++++++ begin substitute gen file loop ++++++++++++++++++++++++++++++++++++=
  for genfile in args.genfiles:

    ioll_gen = larlite.storage_manager(larlite.storage_manager.kREAD)
    ioll_gen.add_in_filename(genfile)
    ioll_gen.open()

    iolcv_gen = larcv.IOManager(larcv.IOManager.kREAD, "larcv", larcv.IOManager.kTickBackward)
    iolcv_gen.add_in_file(genfile)
    #iolcv_gen.reverse_all_products()
    iolcv_gen.initialize()

    #++++++ begin gen entry loop ++++++++++++++++++++++++++++++++++++++++++++++=
    for iEG in range(ioll_gen.get_entries()):

      #ioll_gen.go_to(iEG)
      iolcv_gen.read_entry(iEG)
      evtImage2D_gen = iolcv_gen.get_data(larcv.kProductImage2D, "wiremc")
      adc_v_gen = evtImage2D_gen.Image2DArray()

      #++++++ begin track loop ++++++++++++++++++++++++++++++++++++++++++++++++++=
      for iT in range(nuVertex.track_hitcluster_v.size()):
    
        if iT != args.track_index or not goodTrack(nuVertex.track_v[iT]):
          continue
    
        cropPt = nuVertex.track_v[iT].End()
        if args.contextSub:
          prong_vv = flowTriples.make_cropped_initial_sparse_prong_image_reco_subContextPart(adc_v,
                      adc_v_gen, thrumu_v, nuVertex.track_hitcluster_v[iT], cropPt, args.pixelThresh,
                      args.pixelWH, args.pixelWH, mcpg, args.trackid_rm,  args.trackid_rm2)
        else:
          prong_vv = flowTriples.make_cropped_initial_sparse_prong_image_reco_truthProngSub(adc_v,
                      adc_v_gen, thrumu_v, nuVertex.track_hitcluster_v[iT], cropPt, args.pixelThresh,
                      args.pixelWH, args.pixelWH, mcpg, args.trackid_rm,  args.trackid_rm2)
      ...
      for iS in range(nuVertex.shower_v.size()):
        ...
        if iS != args.shower_index:
          continue
    
        cropPt = nuVertex.shower_trunk_v[iS].Vertex()
        if args.contextSub:
          prong_vv = flowTriples.make_cropped_initial_sparse_prong_image_reco_subContextPart(adc_v,
                      adc_v_gen, thrumu_v, nuVertex.shower_v[iS], cropPt, args.pixelThresh,
                      args.pixelWH, args.pixelWH, mcpg, args.trackid_rm, args.trackid_rm2)
        else:
          prong_vv = flowTriples.make_cropped_initial_sparse_prong_image_reco_truthProngSub(adc_v,
                      adc_v_gen, thrumu_v, nuVertex.shower_v[iS], cropPt, args.pixelThresh,
