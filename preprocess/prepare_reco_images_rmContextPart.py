...
parser.add_argument("-ti", "--track_index", type=int, default=0, help="index for track corresponding to trackid_rm")
parser.add_argument("-si", "--shower_index", type=int, default=-1, help="index for shower corresponding to trackid_rm")
parser.add_argument("-o", "--outfile", type=str, default="prongCNN_partRmContext_images_file.root", help="out file name")
...
for iE in range(ioll.get_entries()):
  ...
  for iT in range(nuVertex.track_hitcluster_v.size()):
    ...
    prong_vv = flowTriples.make_cropped_initial_sparse_prong_image_reco_rmContextPart(adc_v,
                thrumu_v, mcpg, args.trackid_rm, nuVertex.track_hitcluster_v[iT],
                cropPt, args.pixelThresh, args.pixelWH, args.pixelWH)
  ...
  for iS in range(nuVertex.shower_v.size()):
    ...
    prong_vv = flowTriples.make_cropped_initial_sparse_prong_image_reco_rmContextPart(adc_v,
                thrumu_v, mcpg, args.trackid_rm, nuVertex.shower_v[iS],
                cropPt, args.pixelThresh, args.pixelWH, args.pixelWH)
