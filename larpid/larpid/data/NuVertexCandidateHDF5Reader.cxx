...
bool NuVertexCandidateHDF5Reader::readEntry(size_t entry, std::vector<larpid::data::NuVertex>& vertices) {
...
bool NuVertexCandidateHDF5Reader::readTracks( hid_t vtx_group, 
  ...
  if (n_tracks > 0) {
    ...
    vertex.track_hitcluster_v.reserve(n_tracks);
    for (int i = 0; i < n_tracks; ++i) {
      ...
      // Read hit cluster
      auto hitcluster = reconstructLarflowCluster(track_grp);
      if (hitcluster) {
        vertex.track_hitcluster_v.push_back(*hitcluster);
      }
    ...
    if (mom_dataset >= 0) {
      ...
      for (size_t i = 0; i < dims[0]; ++i) {
        ...
        for (size_t j = 0; j < dims[1]; ++j) {
          ...
          //std::vector<float> plane_4mom_v = { flat_data[idx+1], flat_data[idx+2], flat_data[idx+3], flat_data[idx+0] };
          // vertex.shower_plane_mom_vv[i][j].SetPxPyPzE(
          //   flat_data[idx+1], flat_data[idx+2], flat_data[idx+3], flat_data[idx+0]);
          //vertex.shower_plane_mom_vv[i][j] = plane_4mom_v;
        }
      }
...
std::unique_ptr< std::vector< std::vector<float> > > 
NuVertexCandidateHDF5Reader::reconstructTrack(hid_t track_group) {
  auto track = std::make_unique< std::vector< std::vector<float> > >();
  ...
  if (points_dataset >= 0 && n_points > 0) {
    ...
    hsize_t dims[2];
    H5Sget_simple_extent_dims(dataspace, dims, NULL);
    std::cout << "Read track point array. shape=(" << dims[0] << "," << dims[1] << ")" << std::endl;
    ...
    for (size_t i = 0; i < dims[0]; ++i) {
      ...
      std::vector<float> pos = { flat_points[i*3], flat_points[i*3+1], flat_points[i*3+2] };
      track->push_back(pos);
    }
...
std::unique_ptr< std::vector< std::vector<float> > > 
NuVertexCandidateHDF5Reader::reconstructLarflowCluster(hid_t cluster_group) {
  auto cluster = std::make_unique< std::vector< std::vector<float> > >();
  ...
  if (hits_dataset >= 0 && n_hits > 0) {
    ...
    hsize_t dims[2];
    H5Sget_simple_extent_dims(dataspace, dims, NULL);
    std::cout << "Read shower hit array. shape=(" << dims[0] << "," << dims[1] << ")" << std::endl;
    ...
    for (size_t i = 0; i < dims[0]; ++i) {
      ...
      std::vector<float> hit(dims[1],0.0);
      for (size_t j=0; j < dims[1]; ++j ) {
        hit[j] = flat_hits[i*dims[1]+j];
      }
      // hit[0] = flat_hits[i*3];
