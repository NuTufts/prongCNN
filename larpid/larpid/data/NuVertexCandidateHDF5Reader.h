...
namespace data {
  ...
  public:
     ...
     */
    bool readEntry(size_t entry, std::vector<larpid::data::NuVertex>& vertices);
     ...
     * @return True if successful
     */
    bool readVertex(size_t entry, size_t vertex_idx, larpid::data::NuVertex& vertex);
  ...
  private:
    ...
    // Helper methods
    bool readMetadata();
    bool readBasicVertexInfo(hid_t vtx_group, larpid::data::NuVertex& vertex);
    bool readScores(hid_t vtx_group, larpid::data::NuVertex& vertex);
    //bool readClusters(hid_t vtx_group, larpid::data::NuVertex& vertex);
    //bool readClusterPCA(hid_t vtx_group, larpid::data::NuVertex& vertex);
    bool readTracks(hid_t vtx_group, larpid::data::NuVertex& vertex);
    bool readShowers(hid_t vtx_group, larpid::data::NuVertex& vertex);
    ...
    std::unique_ptr< std::vector< std::vector<float> > > reconstructTrack(hid_t track_group);
    std::unique_ptr< std::vector< std::vector<float> > > reconstructLarflowCluster(hid_t cluster_group);
